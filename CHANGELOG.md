...
- **The declared minimum supported Rust version is corrected from 1.85 to 1.86, and CI now enforces it** (PR #54). The old declaration was fiction: the committed lockfile has carried `idna_adapter` 1.2.2 and the `icu_*` 2.2.0 crates (pulled in via `url`) since before v1.6.0, and every one of them declares `rust-version = "1.86"`, so `cargo +1.85 check --locked` exited 101 against exactly the dependency set v1.6.0 shipped with — no 1.85 toolchain could ever have built this crate as published, whatever `Cargo.toml` claimed. The new `MSRV 1.86` CI job proves the declared floor on every pull request (`cargo check --all-targets --all-features --locked` on that toolchain, so the dev-dependency floor — criterion 0.8 also declares 1.86 — is proven too, not just the runtime one), and `tests/msrv_gate_tests.rs` couples the declared number, the job's installed toolchain and the job's name so none of the three can drift alone. For consumers this is a documentation correction, not a new restriction: 1.85 already could not build the crate.

### Fixed

- **`svccat workspace check` now emits byte-identical output for an unchanged workspace** (PR #51). Three separate producers fed `HashMap` iteration order — randomised per process — into the cross-repo dependency report, so ten runs over one unchanged three-repo workspace produced ten distinct documents in `--format json`, ten in `--format html` and ten in `--format markdown`. `unresolvable_dependencies` was built by walking the graph's node map; the circular-dependency search picked its start nodes from that map, and since the node a search enters a cycle from is where that cycle's path is cut, this rotated each cycle's own member list and its description text as well, printing all three rotations of one three-service cycle across ten runs; and the HTML report's dependency-graph payload collected the same map's values, carrying the wander into both its nodes and the links drawn between them. The JSON and Markdown reports are artifacts a CI step diffs or uploads, so this was not a display preference: a byte comparison across runs reported a change that was not one, on a report that names real findings a user is meant to act on. Unresolvable dependencies are now ordered by depending service and then by that service's own `depends_on` declaration order (declaration order is preserved deliberately — it carries information that sorting would discard), cycles are always reported cut at their lowest-sorting member, and the graph payload is sorted by service key. No output content changed, only its order, and no other `--format` value was affected. Library: `deps_graph::ServiceKey` now derives `Ord` and `PartialOrd`, which is additive; `DependencyGraph::nodes` stays a `HashMap` deliberately, because it is a public field and 1.x is API-frozen per `docs/API_STABILITY.md`, so the ordering is imposed where the report is built rather than by changing a published type.
